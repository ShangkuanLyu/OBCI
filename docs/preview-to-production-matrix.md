...
- 分支：`obai-redesign-review` · 基线 HEAD `3770a04` + 本轮工作区修改（未提交）
...
| 12 | `FIXTURE_MEMBERSHIP_NAME_OVERRIDES` 五级会员 DOCX 口径 | `membership_types.name_zh`（`corporate-group`→企业顶级会员、`small`→小微企业会员）、`turnover_zh`（`individual`→自然人创业者） | `121000` §4 | 否 | 应用 §4 | 中文 **已确认**（DOCX 会费表）；手册用词（集团公司会员/小型企业会员/"与专业人士"）被 DOCX 取代；英文名**待确认** |
...
- `news_categories`：6 个分类（商会动态、中澳经贸合作、中澳经贸政策、行业市场资讯、出海实操指南、活动预告回顾）；`中澳经贸合作` 不在 DOCX 五类之内（后台可增分类，属 CMS 数据）。
...
| 资讯中心 | 正常；无行业标签（`tags` 全空）；分类页签只显示有文章的分类 |
...
4. 应用 `20260902120000` → `20260902121000`（顺序不可颠倒）→ `get_advisors` 巡检 → 只读验证 → `generate_typescript_types` → 删除 `as any` 与可选列兜底。
5. 在后台填写：`legal.*_version`（法务批准后；填写前 `_v2` RPC 与页面都会拒绝申请）、联系方式各值及 `contact.confirmed_fields`（后台已可编辑地址/第二地址/电话/传真/手机/邮箱/微信/会员咨询人）、`review.confirmed_modules`、使命/价值观等文本、新闻与活动的行业标签。
6. 替换三个法律页面草拟稿为正式文本。
...
9. 由商会决定：领导简介中的"OBC"旧称是否改写为"协会/OBAI"（简介为手册第 3 页原文，属历史口径）。
